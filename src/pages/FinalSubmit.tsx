...
• A verification of your billing history.`
  ...
  const handleSubmit = () => {
    // When submit is clicked, display the AI reply box
    setShowAiReply(true);
    
    // If there's content in the current AI reply, add it to history
    if (currentAiReply.trim()) {
      ...
      // Clear the current AI reply field
      setCurrentAiReply('');
    }
  };
    ...
    <div className="min-h-screen text-white flex bg-slate-800">
      ...
      <div className="w-[70%] px-10 py-8 flex flex-col ml-[120px]">
        ...
        <div className="mb-10">
            ...
            <div className="mb-6" key={index}>
              <p className="font-semibold text-blue-200 mb-2">
                ▾ {response.type === 'first' ? 'First Response:' : 
                   response.type === 'agl' ? 'AGL\'s Response:' : 
                   'AI Reply:'}
              </p>
              {response.type === 'agl' ? (
                <div className="bg-white text-black text-sm p-4 rounded-md">
          ...
          {/* Current AI Reply Box - Only shown after Submit is clicked */}
          {showAiReply && (
            <div className="mb-6">
              <p className="font-semibold text-blue-200 mb-2">▾ Your Reply:</p>
              <textarea
                className="w-full bg-slate-700 text-white p-4 rounded-md min-h-[150px] resize-y"
                placeholder="Type your reply here..."
                value={currentAiReply}
                onChange={(e) => setCurrentAiReply(e.target.value)}
              />
            </div>
          )}
        </div>
